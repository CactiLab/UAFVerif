...
class RUN:
    ...
    def write_file(self): #write tempfile to analyse
        ...
        f1.close()
        f2.close()
    def simplify(self): # if the result is false, then remove the ! and test if false
        self.setting.regpath = self.setting.regpath[0:-3] + "_rmv.pv"
        self.write_file()
...
class Case:
    def __init__(self,phase):
        ...
        if(phase == "reg"):
            ...
            self.types.append(("autr_1r_em","let atype = autr_1r in\n let ltype = empty in"))
            self.types.append(("autr_1r_st","let atype = autr_1r in\n let ltype = stepup in"))
            self.queries = []
